...
This pipeline reproduces the **structure** of the released SFT data.
The exact prose differs because:

- Skill bullets are LLM-summarized by `aggregate_skills.py` (gpt-4o);
  same input won't produce byte-identical bullets across runs.
- `<think>` reasoning is generated by o3; non-deterministic.

Everything else (system prompt skeleton, conversation structure,
synthetic `done` step construction, action strings, alpaca output
