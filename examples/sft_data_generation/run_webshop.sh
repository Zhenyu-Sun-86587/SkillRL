...
python "${DIR}/skill_memory/generate_memory_webshop.py" \
...
python "${DIR}/distillation/distill_webshop.py" \
  --input_file "${WORK_DIR}/processed_trajectories_cleaned.json" \
...
python "${DIR}/postprocess/sharegpt_to_pairs.py" \
  ...
  --output_file "${WORK_DIR}/webshop_sft_data.json"

echo "==> [4/4] Validate and fix WebShop actions"
python "${DIR}/postprocess/validate_and_fix_webshop.py" \
  --input "${WORK_DIR}/webshop_sft_data.json" \
  --output "${WORK_DIR}/webshop_sft_data_fixed.json"

echo
echo "Done. SFT data: ${WORK_DIR}/webshop_sft_data_fixed.json"
